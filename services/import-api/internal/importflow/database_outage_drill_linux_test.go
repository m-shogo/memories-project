...
// must leave no durable rows, preserve the sealed attempt, and ResumeCommit
// must commit from that same spool after connectivity returns without touching
// object storage or launching another parser worker.
func TestFlowRecoversAfterDatabaseCommitOutage(t *testing.T) {
	...
	// Prove recovery is independent of both source fetch and parser execution.
	env.flow.Committer = healthyCommitter
	env.flow.Objects = nil
	env.flow.Supervisor = nil
	result, err := env.flow.ResumeCommit(context.Background(), request, now)
	if err != nil {
		t.Fatalf("sealed-spool commit did not recover after PostgreSQL returned: %v", err)
	}
	if result.Commit.AlreadyCommitted {
		t.Fatal("first successful recovery commit was mislabeled as a replay")
	}
	if result.Verified.SpoolID != request.SpoolID {
		t.Fatalf("recovery used a different spool: got %s want %s", result.Verified.SpoolID, request.SpoolID)
	}
