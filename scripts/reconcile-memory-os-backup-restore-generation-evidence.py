...
def main() -> int:
    ...
    require(all(isinstance(value, int) for value in (backup_count, restore_count, candidate_count, generation_count)), "recovery/generation counts invalid")
    ...
    if generation_count == 0:
        binding["limitations"] = [
            ...
            "this contract binds future evidence but does not create a backup, provision an environment or execute a restore",
        ...
        binding["limitations"] = [
            ...
            "a production-equivalent recovery candidate does not establish production backup configuration, production traffic safety or application production readiness",
    ...
    append_once(existing, (
        f"{EVIDENCE_PREFIX} registered environment generations={generation_count}, recovery records={count}, complete generation-bound restores={restore_count}, production-equivalent recovery candidates={candidate_count}; exact generation/manifest/artifact hashes, isolated restore, PITR, independent object retention, recovery coherence, non-resurrection, approved objectives and distinct security/operability review are required before a candidate can be derived, while productionEvidence and productionReady remain false"
    ))
    for ref in REFS:
