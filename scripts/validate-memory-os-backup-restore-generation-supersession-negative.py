...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="memory-os-generation-supersession-") as tmp:
        ...
        successor = fixture.generation_record(
        ...
        successor["supersedesGenerationId"] = "pegen_source"
        generations = json.loads(generation_registry.read_text(encoding="utf-8"))
        generations["generations"].append(successor)
        generations["registeredGenerationCount"] = 3
        write_json(generation_registry, generations)
        ...
        try:
            ...
            require(type(exc).__name__ == "Fail", "unexpected exception escaped supersession rejection")
        else:
            raise Fail("new evidence remained admissible after source generation supersession")

    print("Memory OS generation supersession candidate negative suite PASS")
    print("historical evidence remains auditable: true")
    print("superseded generation creates current candidate: false")
