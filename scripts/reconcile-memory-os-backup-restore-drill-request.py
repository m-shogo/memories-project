...
def load_writer():
    spec = importlib.util.spec_from_file_location("memory_os_restore_drill_request_writer_reconcile", WRITER)
...
def expected_decision(generation_count: int, objective_count: int, request_count: int, executable_count: int) -> str:
...
def main() -> int:
    ...
    state["admissionDecision"] = expected_decision(generation_count, objective_count, request_count, executable_count)
    ...
    append_once(
        existing,
        f"{EVIDENCE_PREFIX} registered environment generations={generation_count}, approved recovery objectives={objective_count}, admitted planning requests={request_count}, currently executable requests={executable_count}, admissionDecision={state['admissionDecision']}; admission requires two distinct unsuperseded registered production-equivalent generations, the current approved objective, PITR/WAL and exact object-version policies, all eight planned evidence domains, distinct Recovery Owner/Security/Operability approvals and mandatory stop conditions; historical requests remain auditable after supersession but become non-executable, and request admission never executes a restore or creates production evidence",
    )
    for ref in REFS:
