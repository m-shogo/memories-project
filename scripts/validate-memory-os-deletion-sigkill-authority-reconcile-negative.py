#!/usr/bin/env python3
"""Prove SIGKILL authority identity, atomic publication, mode preservation and rollback remain fail-closed."""
...
def main() -> int:
    ...
    try:
        expect_rejection(module.require_canonical_authorities, "SIGKILL subprocess transport is not canonical")
    finally:
    ...
    try:
        expect_rejection(module.require_canonical_authorities, "SIGKILL atomic writer authority is not canonical")
    finally:
    ...
    try:
        module.CANONICAL_ATOMIC_WRITE_BYTES(
            ...
            b"raise SystemExit('synthetic post-write operability failure')\n",
        )
        rejected = False
        try:
            module.main()
    ...
    print("PASS: SIGKILL authority identity, execution transport, atomic mode and post-write rollback are fail-closed")
    return 0
