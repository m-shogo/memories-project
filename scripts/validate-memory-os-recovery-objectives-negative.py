...
def main() -> int:
    ...
    expect_rejected("arbitrary repository files cannot satisfy objective approval", lambda: writer.validate_record(arbitrary_repo_files))

    objective_binding = copy.deepcopy(valid)
    objective_binding["objectiveId"] = "ro_negative_other_objective"
    expect_rejected("typed approval bound to different objectiveId", lambda: writer.validate_record(objective_binding))

    value_binding = copy.deepcopy(valid)
    value_binding["rpoSeconds"] = 61
    expect_rejected("typed approval bound to different RPO value", lambda: writer.validate_record(value_binding))
