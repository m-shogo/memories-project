...
def atomic_replace_negative(module) -> None:
    ...
    def fail_replace(source, destination) -> None:
        raise OSError("synthetic local logical restore atomic replace rejection")
    ...
    try:
        try:
            ...
            require("synthetic local logical restore atomic replace rejection" in str(exc),
                    f"unexpected atomic replace rejection: {exc}")
        else:
            raise Fail("local logical restore accepted synthetic atomic replace failure")
    finally:
        module.os.replace = real_replace

    require(module.STATUS_PATH.read_bytes() == original_status,
            "production status changed after atomic replace failure")
    require(module.STATUS_PATH.stat().st_mode & 0o7777 == original_mode,
            "production status mode changed after atomic replace failure")
    leftovers = list(module.STATUS_PATH.parent.glob(f".{module.STATUS_PATH.name}.*.tmp"))
    require(not leftovers, f"local logical restore atomic replace left temporary files: {leftovers}")
...
def rollback_failure_diagnostic_negative(module) -> None:
    ...
    source_sha = result.get("commitSha")
    require(isinstance(source_sha, str) and len(source_sha) == 40,
            "logical restore fixture source SHA missing for rollback diagnostic")
    previous_expected_sha = os.environ.get("EXPECTED_COMMIT_SHA")
    calls: list[Path] = []
    ...
    def fail_restore_after_restoring(path: Path, payload: bytes) -> None:
        ...
        if post_write_failure_seen and path == module.STATUS_PATH and payload == original_status:
            ...
            if not rollback_injected:
                ...
                raise OSError("synthetic local logical restore rollback rejection")
            return
        real_atomic_replace(path, payload)
    ...
    try:
        try:
            ...
            require("synthetic aggregate operability rejection" in text,
                    f"primary logical restore failure was lost: {exc}")
            require("rollback incomplete" in text,
                    f"logical restore rollback incompleteness was not reported: {exc}")
            require("synthetic local logical restore rollback rejection" in text,
                    f"logical restore rollback error was lost: {exc}")
        else:
            raise Fail("logical restore reconciler accepted validator plus rollback failure")
        require(rollback_injected, "logical restore rollback failure injection did not execute")
        require(module.STATUS_PATH.read_bytes() == original_status,
                "production status changed after rollback diagnostic failure")
        require(module.STATUS_PATH.stat().st_mode & 0o7777 == original_mode,
                "production status mode changed after rollback diagnostic failure")
    finally:
        module.load = real_load
        module.run_validator = real_run_validator
