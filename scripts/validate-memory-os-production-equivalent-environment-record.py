...
The JSON Schema remains the machine-readable shape contract. This module adds
fail-closed repository/evidence semantics that are intentionally awkward to
express in JSON Schema, and exposes a predicate used by restore-drill preflight.
"""
...
def non_placeholder_text(value: Any, field: str, *, max_length: int) -> str:
...
def validate_environment_record(
    ...
    *,
    ...
    """Validate full shape/semantics and return current preflight eligibility."""
    ...
    non_placeholder_text(topology.get("regionClass"), "topology.regionClass", max_length=64)
    ...
    for index, delta in enumerate(deltas):
        delta = exact_object(delta, {"deltaId", "description", "classification", "accepted", "independentReviewRef"}, f"materialDeltas[{index}]")
        ...
        non_placeholder_text(delta.get("description"), f"materialDeltas[{index}].description", max_length=500)
    ...
    # Every non-null evidence reference is repository-addressable. Equivalent=true
    # additionally requires all of these evidence paths and every semantic control.
    ...
    serialized = json.dumps(env, ensure_ascii=False).lower()
    for forbidden in (
        ...
        require(forbidden not in serialized, f"environment record contains forbidden material: {forbidden}")
