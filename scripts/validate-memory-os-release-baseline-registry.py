...
def validate_record(record: dict[str, Any], required_fields: set[str]) -> None:
    ...
    require(isinstance(approvers, list) and len(approvers) >= 3,
            "release requires at least three approvers")
    roles: set[str] = set()
    ...
    for approver in approvers:
        ...
        require(isinstance(identity, str) and
                re.fullmatch(r"^apr_[a-z0-9][a-z0-9_-]{7,63}$", identity) is not None,
    ...
    require(roles == REQUIRED_ROLES, f"approval roles incomplete: {sorted(REQUIRED_ROLES - roles)}")
    ...
    if rollback.get("status") == "ELIGIBLE":
        require(not conditions, "ELIGIBLE rollback cannot carry unresolved conditions")
        require(rollback.get("verified") is True,
                "ELIGIBLE rollback requires verified=true")
...
def main() -> int:
    ...
    required_fields = set(strings(contract.get("requiredFields"), "requiredFields", 15))
    ...
    strings(contract.get("registrationGuards"), "registrationGuards", 8)
    ...
    require(readiness.get("independentReviewCompleted") is False and
            ...
            "release registry cannot make production ready")

    refs = strings(contract.get("evidenceRefs"), "evidenceRefs", 4)
    for ref in refs:
    ...
    require(isinstance(gate, dict) and gate.get("status") != "READY",
            "empty release registry cannot make OPS-P0-008 READY")
    if not releases:
