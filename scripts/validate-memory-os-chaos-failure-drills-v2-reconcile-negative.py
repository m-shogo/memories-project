...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="memory-os-chaos-v2-negative-") as tmp:
        ...
        if stat.S_IMODE(status_path.stat().st_mode) != original_mode:
            raise RuntimeError("v2 chaos reconcile did not preserve Production Status mode")
        residues = list(root.glob(f".{status_path.name}.*.tmp"))
        if residues:
            raise RuntimeError(f"v2 atomic rollback left temp authority residue: {residues}")
    ...
    with tempfile.TemporaryDirectory(prefix="memory-os-chaos-v2-replace-") as tmp:
        ...
        if stat.S_IMODE(status_path.stat().st_mode) != original_mode:
            raise RuntimeError("v2 atomic replacement failure changed Production Status mode")
        residues = list(root.glob(f".{status_path.name}.*.tmp"))
        if residues:
            raise RuntimeError(f"v2 atomic replacement failure left temp authority residue: {residues}")

    print("PASS: v2 chaos reconcile pins data authority, preserves mode, publishes atomically, and rolls back after aggregate rejection")
    return 0
