...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise EvidenceValidationError(f"missing file: {path.relative_to(ROOT)}") from exc
    except json.JSONDecodeError as exc:
        raise EvidenceValidationError(f"invalid JSON in {path.relative_to(ROOT)}: {exc}") from exc
