...
def expect_mode_preservation(module: ModuleType, original_contract: bytes) -> None:
    ...
    try:
        ...
        if stat.S_IMODE(path.stat().st_mode) != probe_mode:
            raise AssertionError("atomic replacement changed canonical container-kill contract mode")
        if path.read_bytes() != original_contract:
            raise AssertionError("mode-preservation probe changed canonical container-kill contract bytes")
    finally:
...
def expect_rollback_failure_diagnostic(module: ModuleType, original_contract: bytes) -> None:
    ...
    def reject_post_write(_expected_sha: str) -> None:
        def fail_restore(_path: Path, _data: bytes) -> None:
            raise OSError("synthetic container-kill rollback failure")
    ...
    try:
        try:
            ...
            diagnostic = str(exc)
            if "synthetic post-write container-kill validation failure" not in diagnostic:
            ...
            if "synthetic container-kill rollback failure" not in diagnostic:
...
def main() -> int:
    ...
    print("PASS: container-kill reconcile authority, execution transport, mode-preserving atomic writer, rollback, and primary diagnostics are fail-closed")
    return 0
