#!/usr/bin/env python3
"""Negative coverage for local logical restore repository authority containment."""
...
def load_validator():
    spec = importlib.util.spec_from_file_location("memory_os_local_logical_restore_validator", VALIDATOR_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load logical restore validator")
...
def create_side_commit() -> str:
    ...
    return subprocess.check_output(
        ["git", "-c", "user.name=memory-os-negative", "-c", "user.email=memory-os-negative@example.invalid",
         "commit-tree", tree, "-m", "synthetic detached logical restore source"],
        cwd=ROOT,
        text=True,
    ).strip()
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="memory-os-logical-authority-negative-"))
    external_result = temp_root / "result.json"
    external_result.write_bytes(original_bytes if original_bytes is not None else b"{}\n")
    parent_link = ROOT / "docs/fixtures/memory-os-operability/.logical-parent-symlink-negative"
    ...
    try:
        ...
        if original_bytes is not None:
            ...
            expect_fail(
                lambda: validator.validate_result(result, None, 1, 1),
                "not an ancestor",
            )
    finally:
        if result_path.exists() or result_path.is_symlink():
    ...
    print("PASS: logical restore authorities reject symlink escapes and detached source commits")
    return 0
