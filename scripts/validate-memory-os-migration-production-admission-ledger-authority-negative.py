#!/usr/bin/env python3
"""Reject migration admission ledger writer/lock authority substitution."""
...
def main() -> int:
    expect_rejected("writer executable", lambda helper: setattr(helper, "LEDGER_WRITER", ALTERNATE_WRITER))
    expect_rejected("append lock", lambda helper: setattr(helper, "LEDGER_LOCK", ALTERNATE_LOCK))
    print("PASS: migration production admission ledger writer and append-lock substitutions are rejected")
